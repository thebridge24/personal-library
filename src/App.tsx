          ...
          {KEYS.map((key, i) => (
            <button
              ...
              className={`w-[5rem] h-[5rem] rounded-full ${isError ? "animate-shake" : unlocked ? "animate-slide-up" : ""} 
               text-2xl font-medium bg-black/5 active:rounded-xl active:scale-95 transition duration-200 active:bg-black/80 active:text-white`}
            >
              {key}
            </button>
          ))}

         {pin.length > 0 && <button onClick={clearKey} aria-label="clear button" className={`animate-fade ${unlocked ? "animate-slide-up" : ""} absolute left-4 bottom-2 p-4 rounded-full duration-100 `}>
